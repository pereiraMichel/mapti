...
class classeUsuario {
    ...
    public function verificaUltimoId(){
        ...
        try{
            ...
            $sql = "SELECT ULTIMO_ID_USUARIO() AS idUsuario";
//            $sql = "SELECT MAX(idUsuario) AS idUsuario FROM tblusuario";
            $resultado = mysql_query($sql) or die("Problemas ao verificar o último id do usuario");
...
//            mysql_close($resultado);
    ...
    public function inserirUsuario(){
        ...
        if($this->verificaUsuario()){
            ...
            echo "<label class='text-danger'>Este e-mail já existe, favor solicitar ao administrador a alteração da senha.</label><br/>";
            echo "<meta HTTP-EQUIV='refresh' CONTENT='5;URL=index.php?opcao=usuarios'>";
            
        }else{
            $this->verificaUltimoId();
            try{
                $sql = "SELECT INSERIR_USUARIO(".$this->idUsuario.", '".$this->nome."', '".$this->senha."', '".$this->email."', '".$this->dataCadastro."', '".$this->dataAlteracao."', ".$this->codTipoAcesso.", ".$this->codCliente.", ".$this->codStatusUsuario.")"; //Função SQL
...
//                        (".$this->idUsuario.", '".$this->nome."', '".$this->senha."', '".$this->email."', '".$this->dataCadastro."', '".$this->dataAlteracao."', ".$this->codTipoAcesso.", ".$this->codCliente.", ".$this->codStatusUsuario.")";
    ...
    public function alterarUsuario(){
        try{
            $sql = "SELECT ALTERA_USUARIO(".$this->selectIdUsuario.", '".$this->nomeSelecionado."', senha='".$this->senha."', '".$this->email."', '".$this->dataAlteracao."', ".$this->codTipoAcesso.", ".$this->codCliente.", ".$this->codStatusUsuario.")";
    ...
    public function excluirUsuario(){
        try{
            $sql = "SELECT DELETA_USUARIO(".$this->idUsuario.")";
...
//                    echo "      <button class='btn btn-default' width='150' onclick=document.getElementById('idDoUsuario').value='".$dados['idUsuario']."'; type='button' data-toggle='modal' data-target='#alteraUsuario' id='botaoAltera'><i class='fa fa-pencil'></i></button>";//Funcionou
    ...
    public function consultaDadosUsuario(){
        ...
        try{
            ...
            $resultado = mysql_query($sql) or die(ERROQUERY.mysql_error($conecta));//Não aparece o erro.
    ...
    public function getDadosUsuario($opcao){
        switch ($opcao){
            ...
            case 0: //Consulta básica
                ...
                break;
            case 1:
                return //"CALL `CONSULTA_USUARIO_ESPECIFICO`('".$this->idUsuario."')"; 
                    "SELECT  t.idUsuario, t.nome AS nomeUsuario, t.email, t.dataCadastro AS dataUsuario, 
                            ta.tipoAcesso, ta.idTipoAcesso AS codAcesso, c.nome AS nomeCliente, 
                            c.dataCadastro, s.nome as StatusUsuario
                    FROM tblusuario t
            ...
            case 2:
                return "
                    ...
                    INNER JOIN cliente c ON t.codCliente = c.idCliente
                    INNER JOIN statususuario s ON t.codStatusUsuario = s.idStatus
                    WHERE NOT t.idUsuario=".$this->idUsuario;
                break;
            case 3:
                return "
                    SELECT  t.idUsuario, t.nome AS nomeUsuario, t.email, t.dataCadastro AS dataUsuario, 
                ...
                break;
            case 4: // Verifica o e-mail do usuário, se existe
                return "
...
//        echo "<h3>Acesse o botão 'MENU' ao lado para as demais solicitações</h3>";
        echo "<h3>Bem-vindo ao Cloud Support</h3>";
        echo "<br>";
...
//        echo "});</script>"; 
    ...
    public function modalExcluiUsuario(){
        ...
        echo "      </div>";
        echo "      <div class='modal-body'>";
        echo "          <form name='excluiUsuario' method='post' action='index.php?opcao=usuarios&tarefa=exclusao'>";
